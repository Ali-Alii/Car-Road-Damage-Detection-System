...
DATASET_DIR = PROJECT_ROOT / "data" / "road_damage"
...
def ensure_dataset():
    ...
    if missing:
        raise FileNotFoundError("Road dataset missing:\n- " + "\n- ".join(missing))
...
def parse_args():
    ...
    ap.add_argument("--name", default="road_damage_train_v2")
...
def save_metrics(run_dir):
    ...
    (CHECKPOINTS_DIR / "road_metrics.json").write_text(json.dumps(metrics, indent=2), encoding="utf-8")
...
def main():
    ...
    model.train(
        ...
        patience=25, cache=True, cos_lr=True, close_mosaic=15,
        degrees=3, translate=0.06, scale=0.18, fliplr=0.5,
        hsv_h=0.01, hsv_s=0.35, hsv_v=0.25, mixup=0.03
    )
    best_pt = RUNS_DIR / args.name / "weights" / "best.pt"
    if best_pt.exists():
        shutil.copy2(best_pt, CHECKPOINTS_DIR / "road_best.pt")
    model = YOLO(str(CHECKPOINTS_DIR / "road_best.pt"))
    ...
    (CHECKPOINTS_DIR / "road_eval.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    save_metrics(RUNS_DIR / args.name)
    print("Road validation:", summary)
